A summary of `krdict.SubjectCategory`, an enumeration class that contains
Korean Learners' Dictionary subject (theme and situation) categories.

To help navigate the large amount of enumeration values, nested indexing operations
can be used with the top-level categories.

This means that both `SubjectCategory.ELEMENTARY.GREETING` and `SubjectCategory.ELEMENTARY_GREETING`
map to `<SubjectCategory.ELEMENTARY_GREETING: 1>`.
